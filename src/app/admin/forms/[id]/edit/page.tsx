...
import { getFormDetailAction, updateFormAction, generateFormWithAIAction, getUserFormCreationStatusAction } from "@/app/actions/forms";
  ...
  useEffect(() => {
    const fetchForm = async () => {
      ...
      try {
        ...
        console.error("Gagal memuat status premium:", err);
      }
      ...
      if (result.success && result.data) {
        ...
        setIsPaidForm(form.is_paid_form === true);
        setFormPrice(form.form_price || 0);
        setFormPaymentDescription(form.form_payment_description || "");
      ...
      <div className="border-b border-slate-200/80 bg-white py-4 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider bg-slate-50 border border-slate-200 px-2.5 py-1 rounded-lg font-mono truncate max-w-[200px]">
              Edit Form
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Link href={`/forms/${formId}`} target="_blank">
              <Button variant="outline" size="sm" className="border-slate-200 text-slate-600 hover:bg-slate-50 h-9 text-xs rounded-xl font-semibold transition-colors cursor-pointer">
                  ...
                  <CardContent className="p-6 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                      ...
                      <div className="md:col-span-8 space-y-2">
                        <Label htmlFor={`label-${field.id}`} className="text-slate-600 text-[11px] font-bold">Judul Pertanyaan / Label Input</Label>
                        <Input
                          id={`label-${field.id}`}
                    ...
                    {field.type === "file" && (
                      <div className="border-t border-slate-100 pt-4 space-y-2">
                        <Label className="text-slate-600 text-[11px] font-bold">Tipe Berkas yang Diizinkan</Label>
                        <Select
