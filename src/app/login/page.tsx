...
function LoginForm() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        const result = await loginAction(password, email.trim() ? email.trim() : undefined);
        if (result.success) {
        ...
        <form onSubmit={handleLogin}>
          <CardContent className="space-y-4 pt-2">
            <div className="space-y-2">
              <Label htmlFor="email" className="text-slate-600 text-xs font-semibold">
                Email Pengguna <span className="text-slate-400 font-normal">(Kosongkan jika menggunakan password Admin Legacy)</span>
              </Label>
              <Input
                id="email"
                type="email"
          ...
          <CardFooter className="flex flex-col pb-8 pt-4 gap-4">
            ...
            <div className="text-center">
              ...
              <Link href="/register" className="text-xs text-indigo-600 font-bold hover:underline">
                Daftar sekarang
              </Link>
            </div>
          </CardFooter>
